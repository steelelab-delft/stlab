...
class BFWrapper(base_instrument):
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.port = port
        self.htr_ranges = (
            0, 0.0316, 0.1, 0.316, 1., 3.16, 10., 31.6
        )  #For heater current ranges. 0 = Off, 1 = 31.6 μA, 2 = 100 μA, 3 = 316 μA, 4 = 1.00 mA, 5 = 3.16 mA, 6 = 10.0 mA, 7 = 31.6 mA
        super().__init__()
    ...
    def SetControl(
            self, mode=4
    ):  #1 = Closed PID loop, 2 = Zone Tuning, 3 = Open Loop, 4 = off
        """Set Control mode
    ...
    def SetRamp(
            self, mode=True, rate=0.1
    ):  #Set ramp mode and rate (in K/min).  If true, temperature set point will ramp to desired value.  If false, no ramping will be done.
        """Set Ramp mode
    ...
    def SetHeaterRange(
            self, range
    ):  #Specifies heater current range in mA. Will choose the minimum range to include the value given as "range"
        ...
        if rngi < 0:
            print(
                'SetHeaterRange: Error, range too high.  No range change done.'
            )
            return
    ...
    def WaitForTStable(self, channel=6, tol=0.001, timeout=600., tsettle=40.):
        ...
        if success:
            ...
            print("Channel " + str(channel) + " UNSTABLE for " + str(Tset) +
                  ' K')
        return success
