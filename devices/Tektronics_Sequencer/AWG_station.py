...
class AWG_Station():
    ...
    clock = 1e9
    channels_ids = [
        'ch1', 'ch1_marker1', 'ch1_marker2', 'ch2', 'ch2_marker1',
        'ch2_marker2'
    ]
    ...
    def define_channels(self, id, name, type, delay, offset, high, low,
                        active):
    ...
    def program_awg(self, sequence, *elements, **kw):
        ...
        allow_non_zero_first_point_on_trigger_wait = kw.pop(
            'allow_first_zero', False)
        ...
        if (sequence.element_count() > 8000):
            ...
            logging.warning(
                "Error: trying to program '{:s}' ({:d}'".format(
        ...
        print(
            "Programming '%s' (%d element(s)) \t" % (sequence.name,
        ...
        for elt in sequence.elements:
            ...
            if (elt['repetitions'] < 1) or (elt['repetitions'] > 65536):
                raise Exception(
                    'pulsar: The number of repetitions of ' +
                    'AWG element "%s" are out of range. Valid ' % elt['wfname']
                    + 'range = 1 to 65536 ("%s" recieved)' % elt['repetitions']
                )
        ...
        if sequence.djump_table != None and self.AWG_type not in ['opt09']:
            raise Exception(
                'AWG Station: The AWG configured does not support dynamic jumping'
            )
        ...
        if debug:
            self.check_sequence_consistency(packed_waveforms, wfname_l, nrep_l,
                                            wait_l, goto_l, logic_jump_l)

        self.filename = sequence.name + '_FILE.seq'

        # # Loading the sequence onto the AWG memory
        self.AWG.gen_sequence_file(wfname_l[0], wfname_l[1], nrep_l, wait_l,
                                   goto_l, logic_jump_l, self.filename)
    ...
    def check_sequence_consistency(self, packed_waveforms, wfname_l, nrep_l,
                                   wait_l, goto_l, logic_jump_l):
        '''
		Specific for 2 channel tektronix 520 where all channels are used.
		'''
        if not (len(wfname_l[0]) == len(wfname_l[1]) == len(nrep_l) ==
                len(wait_l) == len(goto_l) == len(logic_jump_l)):
            raise Exception(
                'pulsar: sequence list of elements/properties has unequal length'
            )
