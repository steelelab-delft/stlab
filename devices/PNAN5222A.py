...
class PNAN5222A(PNAN5221A):
    ...
    def create_two_sources(self):
        ...
        return PNA_rfsource(self, N_pow=1, N_rang=2), PNA_rfsource(self,
                                                                   N_pow=3,
                                                                   N_rang=4)

    def TwoToneSetup(self, f_probe_start, f_probe_stop, f_probe_points,
                     probe_ifbw, probe_power, f_pump_start, f_pump_stop,
                     f_pump_points, pump_ifbw, pump_power):
        self.write(
            '*RST'
        )  #stop any OPCs and go to preset (same as 'Preset' button) (overkill) (better)
        ...
        ##
        self.write("SENS2:FOM:RANG3:FREQ:CW %s" %
                   (f_probe_start))  #set cw freq to receivers
        self.write("SENS2:FOM:RANG2:FREQ:CW %s" %
                   (f_probe_start))  #set cw freq to source1
        ##
    ...
    def TwoToneProbeForMin(self):
        ...
        self.write('DISP:WIND2:TRAC:Y:PDIV  %s' %
                   eval(self.query('DISP:WIND1:TRAC:Y:PDIV?')))
        self.write('DISP:WIND2:TRAC:Y:RLEV  %s' %
                   eval(self.query('DISP:WIND1:TRAC:Y:RLEV?')))
        self.write('DISP:WIND2:TRAC:Y:RPOS  %s' %
                   eval(self.query('DISP:WIND1:TRAC:Y:RPOS?')))
    ...
    def TwoToneSetProbeFrequency(self, frequency):
        self.write("SENS2:FOM:RANG3:FREQ:CW %s" %
                   (frequency))  #set cw freq to receivers
        self.write("SENS2:FOM:RANG2:FREQ:CW %s" %
                   (frequency))  #set cw freq to source1
    ...
    def GetMin(self):
        ...
        self.write("CALC:MARK1:FUNC:TRAC ON"
                   )  # Don't know what this is but dosnt work without
        self.query('*OPC?')  # Wait
    ...
    def SetSweepType(
            self, tt='LIN'
    ):  #Possible options: LINear | LOGarithmic | POWer | CW | SEGMent | PHASe
