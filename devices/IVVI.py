"""Module for instance of a TUDelft IVVI rack

This module contains the functions necessary to control a TUDelft IVVI rack.
The system only reads and writes short byte arrays.
It does NOT inherit from the "instrument" class.
...
class IVVI_DAC(base_instrument):
    ...
    def numbers_to_mvoltages(self, numbers):
        ...
        '''
        values = [
            (x * 256 + y) / 65535. * 4000. - x0
            for x, y, x0 in zip(numbers[::2], numbers[1::2], self.polmatrix)
        ]
        return tuple(values)
    ...
    def mvoltage_to_bytes(self, mvoltage):
        ...
        if bytevalue > 65535 or bytevalue < 0:
            print("DAC Warning: mvoltage value out of range: ", mvoltage,
                  bytevalue)
    ...
    def SetVoltage(self, dac, mvoltage):
        ...
        (DataH,
         DataL) = self.mvoltage_to_bytes(mvoltage + self.polmatrix[dac - 1])
        #print(DataH, DataL)
        #Prepare byte value message to send
        message = (7, 0, 2, 1, dac, DataH, DataL)
        self.writemessage(message)
        try:
            self.getreply(  # reply = unused variable
            )  #For some reason read_raw does not work.  This command works as a workaround to read X bytes from the device
        except ValueError as error:
            print('Error when setting:' + repr(error))
        setmvoltage = (DataH * 256 + DataL) / 65535. * 4000. - self.polmatrix[
            dac - 1]
        if self.verb:
    ...
    def RampVoltage(
            self, dac, mvoltage, tt=5., steps=100
    ...
    def ReadDACs(self):
        ...
        voltvals = self.numbers_to_mvoltages(
            reply[2:])  #convert the byte values to actual voltage values
        return voltvals
