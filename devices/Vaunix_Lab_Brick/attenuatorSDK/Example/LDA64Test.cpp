...
void PrintHelp()
{
	...
	printf("  -a nn     Set attenuator, nn is attenuation in db units\n");
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	// --- Use the tracing control function to turn on some debug messages
	fnLDA_SetTraceLevel(2, 2, false);
	...
	if (NumDevices > 0)	// do we have a device?
	{
		for (j = 0; j < NumDevices; j++){
			...
			if (!gbQuietMode){
				...
				if (fnLDA_GetFeatures(MyDevices[j]) & HAS_HIRES)
				{
					...
					ftemp = itemp / 100000;	// working frequency is in 100KHz units
					if (itemp >= 0)
						printf("Working Frequency = %.2f db\n", ftemp);
					else
						CheckAPISet(itemp);
				}
		...
		for (iDev = DevNum; iDev < DevNum + DevRange; iDev++)
		{
			...
			if (gbWantSetAttenuation)
			{
				...
				// either the original or the HiRes API functions can be used for any device
				// in each case, scale the attenuation value to the appropriate units

				// using the original API function with .25db units
//				ScaledAttenuation = (int)(Attenuation * 4);
//				itemp = fnLDA_SetAttenuation(MyDevices[iDev], ScaledAttenuation);
